...
const itemVertShaderSource = `#version 300 es
...
const itemFragShaderSource = `#version 300 es
...
// NOVA GEOMETRIA: Plano Retangular Subdividido
class RectGeometry extends Geometry {
  constructor(width = 2.0, height = 1.125, segmentsX = 8, segmentsY = 8) {
    super();
    const hw = width / 2;
    const hh = height / 2;
    ...
    for (let y = 0; y <= segmentsY; y++) {
      const yPos = -hh + y * segmentHeight;
      const v = y / segmentsY;
      for (let x = 0; x <= segmentsX; x++) {
        const xPos = -hw + x * segmentWidth;
        const u = x / segmentsX;
        this.addVertex(xPos, yPos, 0);
...
class InfiniteGridMenu {
  ...
  private itemProgram: WebGLProgram | null = null;
  ...
  private init(onInit?: InitCallback): void {
    ...
    this.itemProgram = createProgram(
      gl,
      [itemVertShaderSource, itemFragShaderSource],
      null,
      {
        aModelPosition: 0,
        aModelNormal: 1,
    ...
    this.itemLocations = {
      aModelPosition: gl.getAttribLocation(this.itemProgram!, 'aModelPosition'),
      aModelUvs: gl.getAttribLocation(this.itemProgram!, 'aModelUvs'),
      aInstanceMatrix: gl.getAttribLocation(
        this.itemProgram!,
        'aInstanceMatrix'
      ),
      uWorldMatrix: gl.getUniformLocation(this.itemProgram!, 'uWorldMatrix'),
      uViewMatrix: gl.getUniformLocation(this.itemProgram!, 'uViewMatrix'),
      ...
      uScaleFactor: gl.getUniformLocation(this.itemProgram!, 'uScaleFactor'),
      uRotationAxisVelocity: gl.getUniformLocation(
        this.itemProgram!,
        'uRotationAxisVelocity'
      ),
      uTex: gl.getUniformLocation(this.itemProgram!, 'uTex'),
      uFrames: gl.getUniformLocation(this.itemProgram!, 'uFrames'),
      uItemCount: gl.getUniformLocation(this.itemProgram!, 'uItemCount'),
      uAtlasSize: gl.getUniformLocation(this.itemProgram!, 'uAtlasSize'),
    };

    // Geometria Retangular 16:9 subdividida
    this.itemGeo = new RectGeometry(2.0, 1.125, 8, 8);
    this.itemBuffers = this.itemGeo.data;
    this.itemVAO = makeVertexArray(
      gl,
      [
        [
          makeBuffer(gl, this.itemBuffers.vertices, gl.STATIC_DRAW),
          this.itemLocations.aModelPosition,
          3,
        ],
        [
          makeBuffer(gl, this.itemBuffers.uvs, gl.STATIC_DRAW),
          this.itemLocations.aModelUvs,
          2,
        ],
      ],
      this.itemBuffers.indices
    );

    this.icoGeo = new IcosahedronGeometry();
    this.icoGeo.subdivide(1).spherize(this.SPHERE_RADIUS);
    this.instancePositions = this.icoGeo.vertices.map((v) => v.position);
    this.ITEM_INSTANCE_COUNT = this.icoGeo.vertices.length;
    this.initItemInstances(this.ITEM_INSTANCE_COUNT);
    this.initTexture();
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        if (imgAspect > TARGET_ASPECT) {
          ...
          sWidth = img.height * TARGET_ASPECT;
          sx = (img.width - sWidth) / 2;
        } else {
          // Imagem é mais alta que 16:9
          sHeight = img.width / TARGET_ASPECT;
  ...
  private initItemInstances(count: number): void {
    if (!this.gl || !this.itemVAO) return;
    const gl = this.gl;
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, this.itemInstances.buffer);
    gl.bufferData(
      gl.ARRAY_BUFFER,
      this.itemInstances.matricesArray.byteLength,
      gl.DYNAMIC_DRAW
    );
    ...
    for (let j = 0; j < mat4AttribSlotCount; ++j) {
      const loc = this.itemLocations.aInstanceMatrix + j;
      gl.enableVertexAttribArray(loc);
  ...
  private animate(deltaTime: number): void {
    ...
    positions.forEach((p, ndx) => {
      ...
      mat4.copy(this.itemInstances.matrices[ndx], matrix);
    });

    this.gl.bindBuffer(this.gl.ARRAY_BUFFER, this.itemInstances.buffer);
    this.gl.bufferSubData(
      this.gl.ARRAY_BUFFER,
      0,
      this.itemInstances.matricesArray
    );
  ...
  private render(): void {
    ...
    gl.uniformMatrix4fv(
      this.itemLocations.uWorldMatrix,
      false,
      this.worldMatrix
    );
    gl.uniformMatrix4fv(
      this.itemLocations.uViewMatrix,
      false,
      this.camera.matrices.view
    );
    gl.uniformMatrix4fv(
      this.itemLocations.uProjectionMatrix,
      false,
      this.camera.matrices.projection
    );
    gl.uniform3f(
      this.itemLocations.uCameraPosition,
      this.camera.position[0],
      this.camera.position[1],
      this.camera.position[2]
    );
    gl.uniform4f(
      this.itemLocations.uRotationAxisVelocity,
    ...
    gl.uniform1i(this.itemLocations.uItemCount, this.items.length);
    gl.uniform1i(this.itemLocations.uAtlasSize, this.atlasSize);

    gl.uniform1f(this.itemLocations.uFrames, this._frames);
    gl.uniform1f(this.itemLocations.uScaleFactor, this.scaleFactor);

    gl.uniform1i(this.itemLocations.uTex, 0);
    ...
    gl.drawElementsInstanced(
      gl.TRIANGLES,
      this.itemBuffers.indices.length,
      gl.UNSIGNED_SHORT,
      0,
      this.ITEM_INSTANCE_COUNT
    );
    gl.bindVertexArray(null);
  }
  ...
  return (
    <div className="relative h-full w-full">
      ...
      {activeItem && (
        <>
          <h2
            className={`-translate-y-1/2 absolute top-1/2 left-[3em] max-w-[6ch] translate-x-[20%] transform select-none font-black text-emerald-500 transition-all ease-[cubic-bezier(0.25,0.1,0.25,1.0)] [font-size:3rem] ${
              isMoving
        ...
        `}
          ...
          <p
            className={`absolute top-1/2 right-[2%] max-w-[17ch] select-none text-balance text-[1.25rem] transition-all ease-[cubic-bezier(0.25,0.1,0.25,1.0)] ${
              isMoving
                ? '-translate-y-1/2 pointer-events-none translate-x-[-60%] opacity-0 duration-[100ms]'
                : '-translate-y-1/2 pointer-events-auto translate-x-[-90%] opacity-100 duration-[500ms]'
            }
        `}
          >
          ...
          <button
            ...
            className={`absolute left-1/2 z-10 grid h-[60px] w-[60px] cursor-pointer place-items-center rounded-full border-[5px] border-black bg-[#00ffff] transition-all ease-[cubic-bezier(0.25,0.1,0.25,1.0)] ${
              isMoving
                ? '-translate-x-1/2 pointer-events-none bottom-[-80px] scale-0 opacity-0 duration-[100ms]'
                : '-translate-x-1/2 pointer-events-auto bottom-[3.8em] scale-100 opacity-100 duration-[500ms]'
            }
        `}
          >
            <p className="relative top-[2px] select-none text-[#060010] text-[26px]">
              &#x2197;
            </p>
          </button>
        </>
